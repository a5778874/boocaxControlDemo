...
import com.iflytek.aiui.uartkit.core.UARTConnector.DataListener;
...
public class UARTManager implements DataListener{
	...
	public void init(final String deviceName, final int speed, final ActionListener listener) {
		...
		mMainExecutor.submit(new Runnable() {
			@Override
			public void run() {
				UARTConnector.getUARTConnector().registerDataListener(UARTManager.this);
				int init_status = UARTConnector.getUARTConnector().init(deviceName, speed);
				if (init_status != -1) {
					listener.onSuccess();
				} else {
	...
	public void destroy() {
		...
		mMainExecutor = null;
		reset();
		UARTConnector.getUARTConnector().destroy();
	}
	...
	private void sendPacket(Packet packet){
		...
		do {
			...
			try {
				...
				int status = UARTConnector.getUARTConnector().send(encodeBytes);
				UARTLogger.log(TAG, "send packet bytes：" + SerialDataUtils.ByteArrToHex(encodeBytes) + " status: " + status);
				if (status == 0) {
					break;
				}
			} catch (UARTException e) {
